...
export function mapError(raw) {
  ...
  const known = {
    ...
    // The commonest real causes of this one are a **demo key used against live** or an
    // **EU-platform key used against global** (or vice versa): OKX runs four separate key
    // universes and a key exists in exactly one. Named that way because "API key doesn't
    // exist" sends people to regenerate a key that was fine all along.
    '50119': 'OKX does not recognise this key here — match "OKX demo trading" and "OKX EU account" in the key modal to where the key was created',
